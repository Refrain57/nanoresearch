...
class BaseVectorStore(ABC):
    ...
    def get_all_documents(
        ...
        **kwargs: Any,
        ...
        Example:
            >>> docs = vector_store.get_all_documents(limit=100)
            >>> for doc in docs:
            ...     print(f"ID: {doc['id']}, Source: {doc.get('source_path')}")
        """
        raise NotImplementedError(
            f"{self.__class__.__name__} does not implement get_all_documents() method. "
            "This operation is used for listing documents in a collection."
        )
