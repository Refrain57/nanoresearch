...
class PdfLoader(BaseLoader):
    ...
    def load(self, file_path: str | Path) -> Document:
        ...
        # Extract bookmarks/TOC for structure-aware chunking
        try:
            bookmarks = self._extract_bookmarks(path)
            if bookmarks:
                metadata["bookmarks"] = bookmarks
                # Rebuild content with Markdown headings from bookmarks
                bookmark_text = self._rebuild_with_bookmarks(path, bookmarks)
                if bookmark_text.strip():
                    text_content = bookmark_text
                    logger.debug(f"Using bookmark-based content for {path.name}")
        except Exception as e:
    ...
    def _extract_bookmarks(self, pdf_path: Path) -> List[Dict[str, Any]]:
        """Extract PDF bookmarks/TOC (Table of Contents).

        Uses PyMuPDF to extract the document's table of contents,
        which provides hierarchical section structure.

        Args:
            pdf_path: Path to PDF file.

        Returns:
            List of bookmark dicts with level, title, page.
        """
        if not PYMUPDF_AVAILABLE:
            logger.warning("PyMuPDF not available, cannot extract bookmarks")
            return []
        ...
        try:
            ...
            if bookmarks:
                logger.info(f"Extracted {len(bookmarks)} bookmarks from {pdf_path.name}")
    ...
    def _rebuild_with_bookmarks(
        self,
        pdf_path: Path,
        bookmarks: List[Dict[str, Any]],
    ) -> str:
        """Rebuild text content with Markdown headings from bookmarks.

        If the PDF has bookmarks/TOC, rebuild the text to include
        Markdown headings (# ## ###) based on the bookmark hierarchy.
        ...
        Returns:
            Text content with Markdown headings inserted.
        """
        if not bookmarks or not PYMUPDF_AVAILABLE:
            return ""  # Will use original MarkItDown output
        ...
        try:
            ...
            for i, bookmark in enumerate(bookmarks):
                ...
                start_page = bookmark["page"] - 1  # 0-indexed
                end_page = bookmarks[i + 1]["page"] - 1 if i + 1 < len(bookmarks) else len(doc)

                # Add Markdown heading
                heading = "#" * min(level, 6)  # Max 6 levels
                lines.append(f"\n{heading} {title}\n")
            ...
            doc.close()
            result = "\n".join(lines)
            logger.info(f"Rebuilt PDF content with {len(bookmarks)} bookmarks as headings")
            return result

        except Exception as e:
            logger.warning(f"Failed to rebuild with bookmarks: {e}")
            return ""
