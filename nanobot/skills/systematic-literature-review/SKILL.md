...
```bash
python $(dirname $(which nanobot))/../skills/systematic-literature-review/scripts/arxiv_search.py \
  "<topic>" \
  --max-results <N> \
  [--category <cat>] \
  [--sort-by relevance] \
...
- **Output path**: Save to `~/.nanobot/workspace/outputs/slr-<topic-slug>-<YYYYMMDD>.md` or use `present_file` to deliver.
