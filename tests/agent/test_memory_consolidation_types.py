...
def _make_mock_knowledge_search():
    """Create a mock KnowledgeSearch with write_user_memory_sync."""
    mock_ks = MagicMock()
...
class TestMemoryConsolidationTypeHandling:
    ...
    async def test_string_arguments_work(self, tmp_path: Path) -> None:
        ...
        mock_ks = _make_mock_knowledge_search()
        store = MemoryStore(tmp_path, knowledge_search=mock_ks)
        provider = AsyncMock()
        ...
        # Verify knowledge_search.write_user_memory_sync was called
        mock_ks.write_user_memory_sync.assert_called_once()
    ...
    async def test_dict_arguments_serialized_to_json(self, tmp_path: Path) -> None:
        ...
        assert result is True
        # Memory update should be serialized to JSON string
    ...
    async def test_missing_memory_update_returns_false_without_writing(self, tmp_path: Path) -> None:
        """Do not write memory if memory_update is missing."""
        mock_ks = _make_mock_knowledge_search()
        store = MemoryStore(tmp_path, knowledge_search=mock_ks)
        provider = AsyncMock()
        provider.chat_with_retry = AsyncMock(
    ...
    async def test_raw_archive_after_consecutive_failures(self, tmp_path: Path) -> None:
        ...
        # Verify raw archive was called on knowledge_search
    ...
    @pytest.mark.asyncio
    async def test_works_without_knowledge_search(self, tmp_path: Path) -> None:
        """Consolidation should still work without knowledge_search (no user_memory write)."""
        store = MemoryStore(tmp_path, knowledge_search=None)
        provider = AsyncMock()
        provider.chat = AsyncMock(
            return_value=_make_tool_response(
                history_entry="[2026-01-01] User discussed testing.",
