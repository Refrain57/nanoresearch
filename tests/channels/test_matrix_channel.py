...
import nanoresearch.channels.matrix as matrix_module
from nanoresearch.bus.events import OutboundMessage
from nanoresearch.bus.queue import MessageBus
from nanoresearch.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from nanoresearch.channels.matrix import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("nanoresearch.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "nanoresearch.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("nanoresearch.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "nanoresearch.channels.matrix.asyncio.create_task", _fake_create_task
    )
