...
from nanoresearch.providers.base import LLMProvider, ToolCallRequest
from nanoresearch.utils.helpers import build_assistant_message
...
class AgentRunner:
    ...
    def _is_failure(result: str) -> bool:
        ...
        if result.startswith("Error:"):
            return True
        exit_code = AgentRunner._parse_exit_code(result)
    ...
    def _extract_error(result: str) -> str:
        ...
        lines = result.split("\n")
        for line in lines:
            if line.startswith("Error:") or "STDERR:" in line:
                return line
        return result[:200]
    ...
    async def run(self, spec: AgentRunSpec) -> AgentRunResult:
        ...
        for iteration in range(spec.max_iterations):
            ...
            if response.has_tool_calls:
                ...
                # P0: 逃生通道 - 检测连续失败
                # 只在单个工具调用时检测（concurrent_tools=False）
                if not spec.concurrent_tools and results:
                    result = results[0] if results else ""
                    if self._is_failure(result):
                        # 记录失败链路
                        self._failure_chain.append({
                            "iteration": iteration,
                            "tool": response.tool_calls[0].name if response.tool_calls else "unknown",
                        ...
                        if response.tool_calls[0].name == self._last_failed_tool:
                            self._consecutive_failures += 1
                        else:
                            self._consecutive_failures = 1
                            self._last_failed_tool = response.tool_calls[0].name

                        if self._consecutive_failures >= spec.max_consecutive_failures:
                            tool_name = response.tool_calls[0].name if response.tool_calls else "unknown"
                            logger.warning(
                                "Agent stuck: consecutive_failures={} on tool={}",
                                self._consecutive_failures, tool_name
                            )
                            return AgentRunResult(
                                final_content=(
                                    f"I'm stuck on `{tool_name}` after {self._consecutive_failures} attempts.\n\n"
                                    f"Last error: {self._failure_chain[-1]['error'][:200]}\n\n"
                                    f"Please help me understand what's wrong, or suggest a different approach."
                                ),
                                messages=messages,
                                tools_used=tools_used,
                                usage=usage,
                                stop_reason="consecutive_failures",
                                error=f"Consecutive failures on {tool_name}",
                                tool_events=self._failure_chain,
                            )
                    else:
                        # 成功，重置计数器
                        self._consecutive_failures = 0
