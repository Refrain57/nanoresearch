...
from nanoresearch.utils.helpers import build_image_content_blocks

if TYPE_CHECKING:
    from nanoresearch.config.schema import WebSearchConfig
...
def _validate_url_safe(url: str) -> tuple[bool, str]:
    ...
    from nanoresearch.security.network import validate_url_target
...
class WebFetchTool(Tool):
        ...
        try:
            async with httpx.AsyncClient(proxy=self.proxy, follow_redirects=True, max_redirects=MAX_REDIRECTS, timeout=15.0) as client:
                async with client.stream("GET", url, headers={"User-Agent": USER_AGENT}) as r:
                    from nanoresearch.security.network import validate_resolved_url
    ...
    async def _fetch_readability(self, url: str, extract_mode: str, max_chars: int) -> Any:
        ...
        from lxml.etree import ParserError, LxmlError
        ...
        try:
            ...
            if "application/json" in ctype:
                ...
                if not r.text or not r.text.strip():
                    return json.dumps({"error": "Empty document", "url": url}, ensure_ascii=False)
                try:
                    doc = Document(r.text)
                    ...
                    logger.warning("readability parsing failed for {}: {}", url, e)
                    text, extractor = r.text[:max_chars], "raw_fallback"
            else:
