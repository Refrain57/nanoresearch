...
class DecomposeToBoardTool(Tool):
    ...
    def description(self) -> str:
        return (
            "把一个可分解为多个不同专长子任务的请求，拆成工作板卡片委派给对应的专长主 Agent，"
            "由各专长主接力/协作完成，最后由你综合回复用户。"
            "【何时使用】当 Agent Registry 中有可用的专长主、且任务明显跨越多个专长领域"
            "（例如既需要深入研究、又需要专业写作/分析/编码/翻译等）时，应优先用本工具把各部分"
            "委派给对应专长主，而不是自己独自完成全部专长工作——让专长主各司其职通常质量更高，"
            "这也是你作为协调者的职责。"
            "【何时不用】纯单领域任务、简单事实问答、闲聊，或 Registry 中没有合适专长主可派时，"
            "直接回答，不要拆卡。"
            "工具调用后协作立即启动；所有卡片完成后你会收到各卡产出并综合回复。"
        )
