...
## ⚠️ nanoresearch 异步等待机制（如果使用 spawn）

如果使用 `spawn` tool 并行执行多个研究任务：
...
**Concurrency limit**: 最多 3 个 spawn 并行。
...
**Round 1 - GitHub API**
Directly execute `scripts/github_api.py`:
```bash
python $(dirname $(which nanoresearch))/../skills/github-deep-research/scripts/github_api.py <owner> <repo> summary
python $(dirname $(which nanoresearch))/../skills/github-deep-research/scripts/github_api.py <owner> <repo> readme
python $(dirname $(which nanoresearch))/../skills/github-deep-research/scripts/github_api.py <owner> <repo> tree
```
