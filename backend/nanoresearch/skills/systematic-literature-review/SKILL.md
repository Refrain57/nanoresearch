...
```bash
python $(dirname $(which nanoresearch))/../skills/systematic-literature-review/scripts/arxiv_search.py \
  "<topic>" \
  --max-results <N> \
  [--category <cat>] \
  [--sort-by relevance] \
...
## ⚠️ nanoresearch 异步等待机制（必须遵守）

**spawn 是异步的**：
- 调用 spawn 后**立即返回** "Subagent started"，不会等待结果
- subagent 完成后会收到系统消息通知
- 通知格式：`[Subagent '<label>' completed successfully]\n\nTask: ...\n\nResult: ...`

**如何判断所有 subagent 已完成**：
1. **先声明期望数量**：在 spawn 之前，明确说 "我将启动 N 个 subagent，label 分别是 X、Y、Z，收到全部 N 个完成通知后再继续"
2. **通过 label 追踪**：给每个 spawn 设置不同的 label
3. **主动计数**：收到通知时计数，收齐 N 个再进入下一批
4. **不要提前继续**：收到 N-1 个通知不能继续，必须等第 N 个

**示例（10 篇论文，分 2 批）**：
```python
# Turn 1: 声明期望
"我将启动 2 个 subagent，label 分别是 batch-1-a 和 batch-1-b，收到全部 2 个完成通知后再继续。"

# 启动第一批
spawn(task="Extract papers 1-5...", label="batch-1-a")
spawn(task="Extract papers 6-10...", label="batch-1-b")

# 返回: "Subagent [batch-1-a] started... Subagent [batch-1-b] started..."
# 此时不要继续，等待通知

# Turn 2-3: 收到通知（可能分散在多次对话中）
# [Subagent 'batch-1-a' completed successfully] Result: [...]
# [Subagent 'batch-1-b' completed successfully] Result: [...]
# → 计数: 2/2，全部到齐

# Turn 4: 所有结果到齐，开始合成或启动下一批
```

---

**You MUST delegate extraction to subagents via the `spawn` tool — do not extract metadata yourself.** This is non-negotiable. Specifically, do NOT do any of the following:
...
- ❌ Use any tool other than `spawn` for this phase

Instead, you MUST call the `spawn` tool to spawn subagents. The reason: extracting 10-50 papers in your own context consumes too many tokens and degrades synthesis quality in Phase 4. Each subagent runs in an isolated context with only its batch of papers, producing cleaner extractions.

Split papers into batches of ~5, then for each batch, call the `spawn` tool. Each subagent receives the paper abstracts as text and returns structured JSON.

**Concurrency limit: at most 3 subagents per turn.** Never dispatch more than 3 spawn calls in the same response.
...
**Parsing subagent results**: the spawn tool returns strings in system messages. When a subagent completes, you will receive a message like:
```
[Subagent 'batch-1-a' completed successfully]

Task: Extract papers 1-5...

Result:
[{"arxiv_id": "1706.03762", ...}, ...]
```
Parse the JSON from after "Result:\n" to the end of the message. If a batch fails or returns unparseable JSON, log it, note which papers were affected, and continue with the remaining batches — do not fail the whole synthesis on one bad batch.
...
- **Prerequisite: `subagent_enabled` must be `true`**. Phase 3 requires the `spawn` tool for parallel metadata extraction. This tool is only available when `subagent_enabled` is set to `true` in the runtime config. Without it, the spawn tool will not appear in the available tools and Phase 3 cannot execute as designed.
- **arXiv only, by design**. This skill does not query Semantic Scholar, PubMed, or Google Scholar. arXiv covers the bulk of CS/ML/physics/math preprints.
...
- **Phase 3 requires subagents to be enabled**. This skill's parallel extraction step hard-requires the `spawn` tool, which is only available when `subagent_enabled=true` at runtime. If subagents are unavailable, do not claim to execute the Phase 3 parallel plan; instead, tell the user that subagents must be enabled for the full workflow, or offer to narrow/split the request into a smaller manual review.
- **Subagent results are in system messages**. The spawn tool immediately returns "Subagent started", but the actual result comes later as a system message. You MUST wait for all expected completion messages before proceeding.
...
- **Output path**: Save to `~/.nanoresearch/workspace/outputs/slr-<topic-slug>-<YYYYMMDD>.md` or use `present_file` to deliver.
