...
from nanoresearch import __logo__, __version__  # noqa: E402
from nanoresearch.cli.stream import StreamRenderer, ThinkingSpinner
from nanoresearch.config.paths import get_workspace_path, is_default_workspace
from nanoresearch.config.schema import Config
from nanoresearch.utils.helpers import sync_workspace_templates
...
def _init_prompt_session() -> None:
    ...
    from nanoresearch.config.paths import get_cli_history_path
...
def onboard(
    ...
    from nanoresearch.config.loader import get_config_path, load_config, save_config, set_config_path
    from nanoresearch.config.schema import Config
    ...
    if wizard:
        from nanoresearch.cli.onboard import run_onboard
...
def _onboard_plugins(config_path: Path) -> None:
    ...
    from nanoresearch.channels.registry import discover_all
...
def _make_provider(config: Config):
    ...
    """
    from nanoresearch.providers.base import GenerationSettings
    ...
    if backend == "openai_codex":
        from nanoresearch.providers.openai_codex_provider import OpenAICodexProvider
...
def _load_runtime_config(config: str | None = None, workspace: str | None = None) -> Config:
    ...
    from nanoresearch.config.loader import load_config, set_config_path
...
def gateway(
    ...
    # Dashboard server
    from nanoresearch.dashboard.server import create_app as _create_dashboard
...
def agent(
    ...
    if logs:
        logger.enable("nanoresearch")
    else:
        logger.disable("nanoresearch")
...
def _get_bridge_dir() -> Path:
    ...
    from nanoresearch.config.paths import get_bridge_install_dir
    ...
    pkg_bridge = Path(__file__).parent.parent / "bridge"  # nanoresearch/bridge (installed)
...
def plugins_list():
    ...
    from nanoresearch.channels.registry import discover_all, discover_channel_names
    from nanoresearch.config.loader import load_config
...
def status():
    ...
    from nanoresearch.config.loader import get_config_path, load_config
    ...
    if config_path.exists():
        from nanoresearch.providers.registry import PROVIDERS
