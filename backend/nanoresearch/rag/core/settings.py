...
def resolve_path(relative: Union[str, Path]) -> Path:
    ...
    - If starts with ~, expand to user home directory
    - If relative, resolve against REPO_ROOT (nanoresearch package directory)
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Settings":
        ...
        if "ingestion" in data:
            ...
            ingestion_settings = IngestionSettings(
                ...
                chunk_strategy=ingestion.get("chunk_strategy", "fixed"),
                min_chunk_length=ingestion.get("min_chunk_length", 100),
...
def load_settings(path: str | Path | None = None) -> Settings:
    ...
    if path is None:
        # Check environment variable (set by MCP server --config flag)
        env_path = _os.environ.get("RAG_SETTINGS_PATH")
        if env_path:
            path = env_path
        else:
    ...
    elif not settings_path.is_absolute():
        settings_path = resolve_path(settings_path)
