"""RAGFlow-like book/structured chunker.

Ported from Yuxi (ragflow_like_v1). Input: markdown string. Output: list[str].
"""
...
from nanoresearch.rag.libs.splitter.ragflow import nlp
...
def chunk_markdown(markdown_content: str, parser_config: dict[str, Any] | None = None) -> list[str]:
    """Split markdown into semantically coherent chunks using hierarchical merging.

    Args:
        markdown_content: Raw markdown text.
        parser_config: Optional dict with keys:
            - chunk_token_num (int, default 512): target tokens per chunk
            - overlapped_percent (int, default 0): overlap percentage
