...
from nanoresearch.rag.core.settings import Settings, load_settings, resolve_path
...
from nanoresearch.rag.libs.loader.markdown_loader import MarkdownLoader
from nanoresearch.rag.libs.loader.marker_loader import MarkerLoader
...
from nanoresearch.rag.ingestion.transform.image_captioner import ImageCaptioner
from nanoresearch.rag.ingestion.transform.doc_metadata_extractor import DocumentMetadataExtractor
...
def _infer_domain(file_path: str, text: str) -> str:
    """Infer the domain/topic of a document based on file path and content.

    Domain classification rules (in order of priority):
    1. Path-based: Check file path for domain indicators
    2. Content-based: Fall back to analyzing document content
    ...
    Returns:
        Domain string: "papers", "nanoresearch", "notes", or "general"
    """
    file_path_lower = file_path.lower()
    text_lower = text.lower()

    # Path-based rules (higher priority)
    path_rules = [
        ("paper", "arxiv", "academic", "research", "journal", "conference", "publication"),
        ("nanoresearch", "ragr", "logr", "mcp", "rag", "agent", "ingestion"),
    ]

    for domain, keywords in [("papers", path_rules[0]), ("nanoresearch", path_rules[1])]:
        for keyword in keywords:
            if keyword in file_path_lower:
                return domain

    # Content-based rules (lower priority, more expensive)
    content_rules = [
        ("papers", [
            "abstract", "introduction", "related work", "methodology", "evaluation",
            "experiment", "conclusion", "citation", "references", "doi",
            "theorem", "proof", "lemma", "corollary", "algorithm"
        ]),
        ("nanoresearch", [
            "chunk", "embedding", "vector", "retrieval", "rag",
            "ingestion", "pipeline", "chroma", "bm25", "hybrid search"
        ]),
    ]

    for domain, keywords in content_rules:
        matches = sum(1 for kw in keywords if kw in text_lower)
        if matches >= 3:  # Need at least 3 keyword matches
            return domain
...
class IngestionPipeline:
    ...
    def run(
        ...
        try:
            ...
            domain = _infer_domain(file_path_str, document.text)
