...
def _register_builtin_providers() -> None:
    ...
    try:
        from nanobot.rag.libs.embedding.openai_embedding import OpenAIEmbedding
        EmbeddingFactory.register_provider("openai", OpenAIEmbedding)
    except ImportError:
        pass  # OpenAI provider not available
    
    try:
        from nanobot.rag.libs.embedding.azure_embedding import AzureEmbedding
        EmbeddingFactory.register_provider("azure", AzureEmbedding)
    except ImportError:
        pass  # Azure provider not available
    
    try:
        from nanobot.rag.libs.embedding.ollama_embedding import OllamaEmbedding
        EmbeddingFactory.register_provider("ollama", OllamaEmbedding)
    except ImportError:
        pass  # Ollama provider not available

    try:
        from nanobot.rag.libs.embedding.dashscope_embedding import DashScopeEmbedding
