...
| 消息持久化 | `Message.tool_calls: JSONB` 随消息存库 | `models.py:132` |
...
### 4.4 存储 —— `Message.citations: JSONB`
- `models.py` Message 新增 `citations: Mapped[list] = mapped_column(JSONB, default=list)`,紧贴 `tool_calls`(`:132`)。
- 在**写入 `Message.tool_calls` 的同一处**一并写 `citations`(同一条 assistant 消息的累积结果)。【需实现时确认:钉死该写入函数】
- 需一条手动 SQL 迁移(`ALTER TABLE messages ADD COLUMN citations JSONB DEFAULT '[]'`),遵循本仓手动迁移惯例。
- 历史加载消息时带出 `citations`(与 `tool_calls` 同路)。
...
kb_search(填 citations)  ──tool result──►  loop.after_iteration
              ...
              ┌───────────────────────────────┤
              ▼                               ▼
   on_citations → xadd_event              写 Message.tool_calls 同处
   {"type":"citations",...}               写 Message.citations(持久化)
              │
              ▼
   SSE → useRunStream(onCitations) → chat store(msg.citations) → MessageList 引用面板
                                                                        ▲
   重进会话：history 加载带出 msg.citations ──────────────────────────────┘
```
...
- **持久化**:`Message.citations` 往返;迁移后旧消息默认 `[]` 不报错;history 加载带出 citations。
...
| Commit | 内容 | 文件 |
|---|---|---|
| C1 | `Message.citations` 列 + 迁移 SQL | `models.py`、迁移脚本 |
| C2 | `kb_search` 填充 `citations` | `rag_search.py` |
| C3 | `loop.py` `on_citations` 捕获/去重/转发 + 签名 | `loop.py` |
| C4 | worker 发 `citations` SSE 事件 + 写入 `Message.citations` | `worker.py` + 消息写入处 |
| C5 | 前端:`useRunStream` 分支 + store 挂载 + `MessageList` 面板 | `useRunStream.js`、chat store、`MessageList.vue` |

顺序:C1→(C2,C3 并行)→C4→C5。每个可单独 revert。
