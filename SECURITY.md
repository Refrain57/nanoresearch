...
If you discover a security vulnerability in NanoResearch, please report it by:
...
**Recommendations:**
- Store API keys in `~/.nanoresearch/config.json` with file permissions set to `0600`
...
- ✅ Never run NanoResearch as root
...
- ✅ Run NanoResearch with a dedicated user account
...
- Keep authentication data in `~/.nanoresearch/whatsapp-auth` secure (mode 0700)
...
pip install --upgrade nanoresearch-ai
```
...
- Subscribe to security advisories for NanoResearch and its dependencies
...
2. **Use a Dedicated User**
   ```bash
   sudo useradd -m -s /bin/bash nanoresearch
   sudo -u nanoresearch nr gateway
   ```
...
4. **Enable Logging**
   ...
   tail -f ~/.nanoresearch/logs/nanobot.log
   ```
...
- **Chat history is stored locally** - protect the `~/.nanoresearch` directory
...
2. **Review logs for unauthorized access**
   ```bash
   grep "Access denied" ~/.nanoresearch/logs/nanobot.log
   ```
